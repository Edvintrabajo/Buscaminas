...
function numeros(casilla) {
    nbombas = 0;
    fila = parseInt(casilla.id.charAt(0));
    columna = parseInt(casilla.id.charAt(2));
    for (i = Math.max(0, fila - 1); i <= Math.min(7, fila + 1); i++) {
        for (j = Math.max(0, columna - 1); j <= Math.min(7, columna + 1); j++) {
...
function revelarCasillasVecinas(casilla) {
    if (casilla.classList.contains("cerrado")) {
        ...
        fila2 = parseInt(casilla.id.charAt(0));
        columna2 = parseInt(casilla.id.charAt(2));
        for (k = Math.max(0, fila2 - 1); k <= Math.min(7, fila2 + 1); k++) {
            for (l = Math.max(0, columna2 - 1); l <= Math.min(7, columna2 + 1); l++) {
