...
function numeros(casilla) {
    ...
    for (i = Math.max(0, fila - 1); i <= Math.min(23, fila + 1); i++) {
        for (j = Math.max(0, columna - 1); j <= Math.min(23, columna + 1); j++) {
...
function revelarCasillasVecinas(casilla) {
    if (casilla.classList.contains("cerrado")) {
        ...
        for (k = Math.max(0, fila2 - 1); k <= Math.min(23, fila2 + 1); k++) {
            for (l = Math.max(0, columna2 - 1); l <= Math.min(23, columna2 + 1); l++) {
